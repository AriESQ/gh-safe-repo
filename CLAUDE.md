...
- Uses the user's own git credentials by default, NOT the OAuth token; URLs follow `git_protocol` (`ssh` → `git@github.com:owner/repo.git`, else plain HTTPS via credential helper). Blanket token injection (`x-access-token`) was removed because OAuth App tokens require the `workflow` scope to push `.github/workflows/*`, even when the user's SSH key has no such restriction
- `[git_transport] mode = auto/user_creds/token` (Rec C) restores token-in-URL injection as a narrow fallback: `auto` (default) injects only when HTTPS + no credential.helper + token available (CI/headless, P4/P6); SSH users are never switched. `user_creds` pins the workflow-scope-free push; `token` forces HTTPS+token and skips discovery. Token-bearing URLs are redacted from all output (`transport.redact()`), and `persistent_url()` (clean, token-free) is what gets written to the user's `.git/config` — never `remote_url()`
